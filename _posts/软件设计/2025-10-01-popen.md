---
title: 使用 popen 代替 system 函数执行系统命令
category: [软件设计]
tags: [popen, system, command, cmd]
---

> 在 c/c++ 编程中常常需要执行一些系统命令，可以使用 system 函数，也可以采用 popen 的方式来实现，本文介绍 popen 的使用方法及优势。
{: .prompt-info }

## system vs popen

| 特性 | system | popen |
|------|--------|-------|
| 获取输出 | ❌ 无法获取 | ✅ 可实时读取 |
| 灵活性 | ❌ 简单粗暴 | ✅ 逐行处理 |
| 错误处理 | ❌ 复杂 | ✅ 精确状态码 |
| 效率 | ❌ 低（启动shell） | ✅ 高（直接通信） |

## 实现详解

command.h

```c++
#ifndef COMMAND_H
#define COMMAND_H

#include <stdio.h>
#include <sys/wait.h>
#include <string>
#include <sstream>
#include <functional>

class Command {
public:
    using Callback = std::function<void(const std::string &output)>;

public:
    explicit Command(
        const std::string &command,
        const Callback &callback = nullptr)
        : command_(command),
          callback_(callback) {}

    int execute() {
        std::stringstream output;
        /// 使用 popen 执行命令并获取输出
        fp_ = popen(command_.c_str(), "r");
        if (!fp_) {
            return -1;
        }
        char buffer[4096] = {0};
        /// 读取命令输出，直到结束，一次读取一行，存入 output
        while (nullptr != fgets(buffer, sizeof(buffer), fp_)) {
            output << buffer;
        }
        /// 调用回调函数处理输出
        if (callback_) {
            callback_(output.str());
        }
        /// 关闭管道并获取命令执行状态
        int status = pclose(fp_);
        fp_ = nullptr;
        /// 关闭 pclose 是否失败
        if (-1 == status) {
            return -2;
        }
        /// 命令未正常退出
        if (!WIFEXITED(status)) {
            return -3;
        }
        /// 返回命令退出状态码
        return WEXITSTATUS(status);
    }

private:
    FILE *fp_ = nullptr;
    Callback callback_;
    std::string command_;
};
#endif // COMMAND_H
```

main.cpp

```cpp
#include "command.h"
#include <iostream>

int main() {
    Command cmd("ls -l", [](const std::string &output) {
        std::cout << output;
    });

    int result = cmd.execute();
    if (result < 0) {
        std::cerr << "Command execution failed with code: " << result << std::endl;
    }

    return 0;
}
```

## 代码说明

### 核心步骤

| 步骤 | 说明 |
|------|------|
| popen | 打开命令的标准输出管道 |
| fgets | 逐行读取输出 |
| Callback | 处理输出结果 |
| pclose | 关闭管道获取状态码 |
| WIFEXITED/WEXITSTATUS | 校验和提取退出状态 |

## 注意事项

- **命令注入**：避免直接拼接用户输入，使用白名单验证
- **缓冲区溢出**：根据需求调整缓冲区大小
